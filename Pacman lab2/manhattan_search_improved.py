...
def manhattan_search_improved(start, end):
...
def _find_adjacent_cells(curr, visited_map):
    ...
    print_debug = len(adjacent_cells_with_coins) > 1
    adjacent_cells_with_coins.sort(key=lambda x: find_coin_heuristic_value(x, print_debug))
    if print_debug:
        print(adjacent_cells_with_coins)
...
def find_coin_heuristic_value(coin_position, print_debug):
    value = 0
    for i in range(len(map.coins_map)):
        for j in range(len(map.coins_map[i])):
            if map.coins_map[i][j] and i != coin_position.y and j != coin_position.x:
                value += abs(coin_position.y - i) + abs(coin_position.x - j)
    if print_debug:
        print(coin_position, value)
    return value
